...
class SignalMetrics:
    ...
    def growing_factor(self):
        try:
            ...
            initial_guess = [max(self._signal_before_peak), 1, np.median(self._time_before_peak), 0]
            ...
            popt, _ = curve_fit(logistic_function, self._time_before_peak, self._signal_before_peak,
                                p0=initial_guess, bounds=bounds, maxfev=10000)  # Increased maxfev
            ...
            ss_tot = np.sum((self._signal_before_peak - np.mean(self._signal_before_peak)) ** 2)
            # Residual sum of squares (SS_res)
            ss_res = np.sum(residuals ** 2)
            # R-squared (R²) value
            r_squared = 1 - (ss_res / ss_tot)
            # Convert the parameters to regular floats and round them
            return round(float(k), 3), round(float(r_squared), 3), np.round(fitted_values, 3).tolist(), self._time_before_peak
    ...
    def growing_rate(self):
        ...
        try:
            ...
            peak_signal = np.max(self._signal_before_peak)  # Peak value of the signal before the peak
            peak_time = self._time_before_peak[np.argmax(self._signal_before_peak)]  # Time at which the peak occurs
    ...
    def growing_rate_speed(self, rise_index):
        ...
        growing_rate = (abs(signal_at_rise) - abs(signal_at_start)) / (time_at_rise - time_at_start)
    ...
    def get_rising_time(self, target_signal):
        ...
        for i in range(len(self.signal)):
            ...
            if rise_time is None and self.signal[i] >= RISETIME_REACHED_RATIO * target_signal[i]:
    ...
    def get_gr_before_rising_time(self, rise_index):
        ...
        # Compute the growing rate as change in signal over change in time
        growing_rate = (signal_at_rise - signal_at_start) / (time_at_rise - time_at_start)
